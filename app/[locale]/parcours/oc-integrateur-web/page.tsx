...
export default async function OcIntegrateurWebDetailPage() {
    const tTopBar = await getTranslations('parcoursDetails');
    const t = await getTranslations('parcoursDetails.webIntegrator');
                ...
                <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
                    ...
                    <nav aria-label="Parcours" className="flex gap-1 rounded-xl bg-muted/50 p-1">
                        {PARCOURS_LINKS.map((item) => {
                            const isActive = item.href === '/parcours/oc-integrateur-web';
                            return (
                                <Link
                                    key={item.href}
                                    href={item.href}
                        ...
                        <div className="space-y-8">
                            <div>
                                ...
                                <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-muted-foreground">
                                    <li>
                                        {t.rich('missions.puce1', {
                                            strong: (chunks) => <strong
                                                className="font-semibold text-foreground">{chunks}</strong>
                                        })}
                                    </li>
                                    <li>
                                        {t.rich('missions.puce2', {
                                            strong: (chunks) => <strong
                                                className="font-semibold text-foreground">{chunks}</strong>
                                        })}
                                    </li>
                                    <li>
                                        {t.rich('missions.puce3', {
                                            strong: (chunks) => <strong
                                                className="font-semibold text-foreground">{chunks}</strong>
                                        })}
                                    </li>
                                    <li>
                                        {t.rich('missions.puce4', {
                                            strong: (chunks) => <strong
                                                className="font-semibold text-foreground">{chunks}</strong>
                                        })}
                                    </li>
                                    <li>
                                        {t.rich('missions.puce5', {
                                            strong: (chunks) => <strong
                                                className="font-semibold text-foreground">{chunks}</strong>
                                        })}
                                    </li>
