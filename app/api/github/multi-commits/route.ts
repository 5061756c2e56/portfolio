...
async function fetchAllRepoCommits(
    ...
    while (hasMore && page <= maxPages) {
        try {
            const result = await getCommitsList(
                repo.owner,
                repo.name,
                startDate,
                undefined,
            ...
            console.error(
                `[Multi-commits] Error fetching page ${page} for ${repo.name}:`,
                error
            );
            break;
        }
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (useDB) {
            try {
                const dbResult = await getCommitsFromDB(
                    validRepos,
                    range,
                ...
                console.warn(
                    '[Multi-commits] DB query failed, falling back to API:',
                    dbError
                );
            }
        }
        ...
        const results = await Promise.all(
            validRepos.map((repo) =>
                fetchRepoCommits(repo, startDate.toISOString(), searchQuery)
            )
        );

        const commitsByRepo: Record<
            string,
            {
                displayName: string;
                commits: MultiRepoCommitItem[];
